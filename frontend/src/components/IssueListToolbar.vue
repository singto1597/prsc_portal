  ...
  <div ref="rootEl" class="flex flex-wrap items-center gap-2">
    ...
    <div class="relative flex-1 min-w-[200px] sm:flex-none sm:w-72">
      ...
      <input
        ...
        class="w-full pl-9 pr-8 py-2.5 border border-gray-300 rounded-xl text-sm bg-white focus:ring-2 focus:ring-red-500 focus:border-red-500"
      />
      <button
        v-if="q"
        type="button"
    ...
    <div class="relative">
      ...
      <Transition name="pop">
        <div
          v-if="open"
          class="absolute right-0 top-full mt-2 w-72 sm:w-80 bg-white border border-gray-200 rounded-2xl shadow-lg z-30 p-4"
        >
          <slot name="filters" />
          ...
          <div class="mt-4 pt-3 border-t border-gray-100">
            ...
            <div class="grid grid-cols-2 gap-1 p-1 bg-gray-100 rounded-xl">
              <button
                type="button"
                @click="sort = 'desc'"
                :class="sort === 'desc' ? 'bg-white shadow text-red-600' : 'text-gray-500 hover:text-gray-700'"
              ...
              <button
                type="button"
                @click="sort = 'asc'"
                :class="sort === 'asc' ? 'bg-white shadow text-red-600' : 'text-gray-500 hover:text-gray-700'"
                class="px-3 py-1.5 rounded-lg text-sm font-medium transition"
              >
                เก่าไปใหม่
              </button>
    ...
    <!-- จำนวนเรื่อง -->
    <span class="text-sm text-gray-400 ml-auto tabular-nums" :class="{ 'opacity-50': loading }">
